...
export class Neodev {
  ...
  async downloadTypes(): Promise<void> {
    ...
    await promisify(exec)(
      `git clone https://github.com/folke/neodev.nvim.git ${this.repoDir} --depth=1`,
    );
    if (this.typesExist)
      window.showInformationMessage(`Download nvim lua types successfully!`);
    else window.showErrorMessage("Download nvim lua types failed!");
  }
  ...
  async getTypesPath(): Promise<string> {
    ...
    if (this.typesExist) {
      return path.join(
        ...
        workspace.isNvim &&
          Boolean(
            await workspace.nvim.call("luaeval", "vim.version().prerelease"),
          )
          ? "nightly"
          : "stable",
      );
